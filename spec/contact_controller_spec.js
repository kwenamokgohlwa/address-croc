...
describe("ContactController", () => {
  ...
  describe("#addContact()", () => {
    it("should add a single contact into the book", (done) => {
      this.book.addContact("Alice", "001-101-1010", "alice@wonderland.com").then((contact) => {
  ...
  describe("#getContacts()", () => {
    ...
    it("should return an array of contacts when contacts are available", (done) => {
      this.book.addContact("Alice", "001-101-1010", "alice@wonderland.com").then(() => {
        this.book.getContacts().then((contacts) => {
  ...
  describe("search methods", () => {
    ...
    describe("#binarySearch()", () => {
      ...
      it("should return null when called with empty array", () => {
        expect(this.book.binarySearch([], "Alloy Rodrguez")).toBeNull();
      });
    ...
    describe("#search()", () => {
      it("should return null when the contact was not found", (done) => {
        this.book.addContact(...zelda).then(() => {
  ...
  describe("#delete()", () => {
    it("should not remove any contacts that do not match the ID passed", (done) => {
      this.book.addContact("Rick Deckard", "000-000-0000", "null@null.com").then(() => {
        this.book.getContacts().then((contacts) => {
    ...
    it("should remove the contact that matches the ID passed", (done) => {
      this.book.addContact("Rick Deckard", "000-000-0000", "null@null.com").then((contact) => {
        this.book.getContacts().then((contacts) => {
          expect(contacts[0].name).toBe("Rick Deckard");
          expect(contacts.length).toBe(1);
